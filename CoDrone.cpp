...
void CoDroneClass::LedColorProcess(byte _dType, byte sendMode, byte sendColor, byte sendInterval)
{		
	byte _packet[9];
	byte _crc[2];
...
{		
	byte _cType = dType_LedMode;
	LedColorProcess(_cType, sendMode, sendColor, sendInterval);	
}

void CoDroneClass::LedColor(byte sendMode, byte r, byte g, byte b, byte sendInterval)
{		
	byte _cType = dType_LedModeColor;
	LedColorProcess(_cType, sendMode, r, g, b,sendInterval);
}
...
{	
	byte _cType = dType_LedDefaultColor;
	LedColorProcess(_cType, sendMode, r, g, b,sendInterval);	
}
...
{	
	...
	byte _dType = dType_LedDefaultColor2;
	byte _len   = 0x0A;  
          ...
          {        
			...
			for(int i = 0; i < receiveLength + 5 ; i++)	dataBuff[i - 3] = cmdBuff[i - 1];   
              ...
              {                       	              	
               ...
               else if (receiveDtype == dType_Range)//dron Range
               {         
               ...
               else if (receiveDtype == dType_CountFlight)//dron CountFlight
               {
                	receiveCompleteData[0] = dataBuff[2];
                	receiveCompleteData[1] = dataBuff[3];	
                	receiveCompleteData[2] = dataBuff[4];	
                 ...
                 else if (receiveDtype == dType_CountDrive)//dron CountDrive
                {
                	/*u64 timeDrive;              // 주행 시간        
                ...
                else if ((receiveDtype == dType_Pressure) || (receiveDtype ==  dType_Battery) ||  (receiveDtype ==  dType_Motor))	//dron Pressure || dron Battery || dron Motor
                {
                	receiveCompleteData[0] = dataBuff[2];
                	receiveCompleteData[1] = dataBuff[3];	
                	receiveCompleteData[2] = dataBuff[4];	
...
void CoDroneClass::LinkStateCheck()	//ready or connected ?
{
	...
	delay(50);
	PreviousMillis = millis();
	while ((linkState <= 0) && (!TimeCheck(500))) 	
	{
		Receive();
	}  
}
...
void CoDroneClass::ReceiveEventCheck(byte _completeData[])
{
	 ...
	 /**************************************************************/		
		...
		else if(receiveDtype == dType_Pressure)
		{
			  ...
			  { 			  		
			  	...
			  	long temperature	= ((_completeData[11] << 32) |(_completeData[10] << 16) |(_completeData[9] << 8) | (_completeData[8]  & 0xff));
			  	long pressure		= ((_completeData[15] << 32) |(_completeData[14] << 16) |(_completeData[13] << 8) | (_completeData[12]  & 0xff));	  						  			
		...
		else if (receiveDtype ==  dType_Battery)
		{
			  ...
			  { 			  		
				...
				DEBUG_SERIAL.print(_completeData[15],HEX);										
			  	...
			  	int v30			= ((_completeData[1] << 8) | (_completeData[0]  & 0xff));
			  	int v33			= ((_completeData[3] << 8) | (_completeData[2]  & 0xff));			  	 	
			  	int gradient	= ((_completeData[5] << 8) | (_completeData[4]  & 0xff));
			  	int yIntercept	= ((_completeData[7] << 8) | (_completeData[6]  & 0xff));
			  	int flagBatteryCalibration	= _completeData[8];			  	
			  	long batteryRaw	= ((_completeData[12] << 32) |(_completeData[11] << 16) |(_completeData[10] << 8) | (_completeData[9]  & 0xff));			  
			  	int batteryPercent	= _completeData[13];			  	
			  	int voltage	= ((_completeData[15] << 8) | (_completeData[14]  & 0xff));
			  	
			  	
			  	DEBUG_SERIAL.print("v30\t");	
				DEBUG_SERIAL.println(v30);
					
				DEBUG_SERIAL.print("v33\t");	
				DEBUG_SERIAL.println(v33);
				
			  	DEBUG_SERIAL.print("gradient\t");	
				DEBUG_SERIAL.println(gradient);
				
				DEBUG_SERIAL.print("yIntercept\t");	
				DEBUG_SERIAL.println(yIntercept);
	    ...
	    {	    	
			...
			if(debugMode == 1)
			{
				...
				/*		
			  	int left		= ((_completeData[1] << 8) | (_completeData[0]  & 0xff));
			  	int front		= ((_completeData[3] << 8) | (_completeData[2]  & 0xff));	
				int right		= ((_completeData[5] << 8) | (_completeData[4]  & 0xff));
			  	int real		= ((_completeData[7] << 8) | (_completeData[6]  & 0xff));	
				int top			= ((_completeData[9] << 8) | (_completeData[8]  & 0xff));
			  	int bottom		= ((_completeData[11] << 8) | (_completeData[10]  & 0xff));	
				*/		
		...
		else if (receiveDtype == dType_Temperature)
    {
			  ...
			  { 			  		
				...
				DEBUG_SERIAL.print(_completeData[7],HEX);										
			  	...
			  	long imu0		= _completeData[0];
			  	long imu1		= _completeData[1];
			  	long imu2		= _completeData[2];
			  	long imu3		= _completeData[3];
			  	
			  	long pressure0	= _completeData[4];
			  	long pressure1	= _completeData[5];
			  	long pressure2	= _completeData[6];
			  	long pressure3	= _completeData[7];			  				  	
			  	
			  	long imu_temp		= ((imu3 << 32) |(imu2 << 16) |(imu1 << 8) | (imu0  & 0xff));
			  	long pressure_temp	= ((pressure3 << 32) |(pressure2 << 16) |(pressure1 << 8) | (pressure0  & 0xff));
	...
	/**************************************************************/						
	  ...
	  else if ((((receiveDtype == dType_LinkEvent) || (receiveDtype == dType_LinkEventAddress))&& (receiveEventState > 0)))
	  {  	
